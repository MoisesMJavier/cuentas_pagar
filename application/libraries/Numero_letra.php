<?php
class Numero_letra extends CI_Model {
  ...
  private $MONEDAS = array(
    ...
    public function to_word($number, $miMoneda = null) {
        if (strpos($number, $this->separator) === FALSE) {
          $convertedNumber = array(
            $this->convertNumber($number, $miMoneda, 'entero').' MN'
          );
        } else {
          $number = explode($this->separator, trim($number));
          $convertedNumber = array(
    ...
    private function convertNumber($number, $miMoneda = null, $type) {   
        ...
        ($number < 2 ? $moneda = 'PESO' : $moneda = 'PESOS');
