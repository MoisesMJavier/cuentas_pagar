...
class M_DireccionGeneral extends CI_Model
{
    ...
    function obtenerSolPendientesOtros()
    {
        ...
        /** FECHA: 06-JUNIO-2025 | NUEVO CONCEPTO SOLO DEPTO CH | @author Angel Victoriano <programador.analista30@ciudadmaderas.com> **/
        return $this->db->query("SELECT 
            IFNULL(notifi.visto, 1) AS visto,
    ...
    function obtenerSolPendientes()
    {
        ...
        return $this->db->query(/*"SELECT proveedores.idproveedor,proveedores.nombre AS Proveedor, MIN( IF(solpagos.programado IS NULL, solpagos.fecelab, CASE WHEN solpagos.programado < 7 THEN DATE_ADD( solpagos.fecelab, INTERVAL ( IFNULL(prealizados, 0) * solpagos.programado ) MONTH ) ELSE DATE_ADD( solpagos.fecelab, INTERVAL IFNULL(prealizados, 0) WEEK ) END ) ) AS FECHAFACP, SUM( solpagos.cantidad ) - SUM( IF( solpagos.programado IS NULL, IFNULL(abonado, 0), 0 ) ) Cantidad FROM solpagos INNER JOIN proveedores ON proveedores.idproveedor = solpagos.idProveedor LEFT JOIN ( SELECT autpagos.idsolicitud, COUNT(autpagos.idpago) prealizados, SUM( autpagos.cantidad ) abonado FROM autpagos WHERE autpagos.estatus IN ( 14, 15, 16 ) GROUP BY autpagos.idsolicitud ) AS abono ON abono.idsolicitud = solpagos.idsolicitud WHERE solpagos.idetapa IN ( 7, 9) AND (solpagos.caja_chica IS NULL OR solpagos.caja_chica = 0) AND solpagos.nomdepto NOT IN ( 'DEVOLUCIONES', 'PRESTAMO', 'FINIQUITO', 'FINIQUITO POR RENUNCIA', 'FINIQUITO POR PARCIALIDAD', 'BONO','TRASPASO','DEVOLUCION', 'IMPUESTOS', 'NOMINA DESTAJO' ) AND solpagos.metoPago NOT IN ( 'FACT BAJIO', 'FACT BANREGIO' ) GROUP BY proveedores.idproveedor ORDER BY proveedores.nombre, solpagos.fecelab"*/
                    "SELECT
                        ...
                        provCredito.dias_credito,
                        provCredito.monto_credito
                    FROM
                        ( 
                            SELECT
                        ...
                        LEFT JOIN proveedores AS provCredito
	                        ON prov.rfc = provCredito.rfc AND UPPER(TRIM(prov.nombre)) = UPPER(TRIM(provCredito.nombre)) AND provCredito.estatus IN (1,4)
    ...
    function obtenerSolDiferida()
    {
        ...
        return $this->db->query("SELECT 
            ...
            solpagos.moneda, 
            CONCAT( SUBSTRING( solpagos.justificacion, 1, 150 ), '...' ) AS Observacion, 
        ...
        WHERE solpagos.idetapa IN ( 7, 9 ) 
        AND (solpagos.caja_chica IS NULL OR solpagos.caja_chica = 0)
        AND solpagos.nomdepto NOT IN ( 'DEVOLUCIONES', 'PRESTAMO', 'FINIQUITO', 'FINIQUITO POR RENUNCIA', 'FINIQUITO ASIMILADO', 'FINIQUITO ASIMILADO', 'FINIQUITO POR PARCIALIDAD', 'BONO','TRASPASO','DEVOLUCION', 'IMPUESTOS', 'NOMINA DESTAJO' ) 
        AND solpagos.metoPago NOT IN ( 'FACT BAJIO', 'FACT BANREGIO' ) 
    ...
    */
		...
		return $this->db->query("SELECT 
            ...
            FROM ( 
                SELECT 
                    ...
                    SUM(cantidad) Cantidad
                FROM solpagos 
                WHERE caja_chica = 3 AND idetapa IN ( 7 )
                GROUP BY idResponsable, idEmpresa
            ) s
            INNER JOIN listado_usuarios u ON s.idResponsable =  u.idusuario
            INNER JOIN empresas emp ON emp.idEmpresa = s.idEmpresa
		...
		return $this->db->query("SELECT 
            ...
            FROM ( 
                SELECT 
                    ...
                    SUM(cantidad) Cantidad
                FROM solpagos 
                WHERE caja_chica = 4 AND idetapa IN ( 7 )
                GROUP BY idResponsable, idEmpresa
            ) s
            INNER JOIN listado_usuarios u ON s.idResponsable =  u.idusuario
            INNER JOIN empresas emp ON emp.idEmpresa = s.idEmpresa
    ...
    function getSolicitudesCCH($idsolicitudes)
    {
        return $this->db->query("SELECT 
                                    s.idsolicitud
                                    , s.cantidad
                                    , IFNULL(s.proyecto, pd.nombre) AS proyecto
                                    , IFNULL(s.etapa, '') AS etapa
                                    , IFNULL(s.condominio, '') AS condominio
                                    , p.nombre AS proveedor
                                    , DATE_FORMAT(s.fecelab,'%d/%b/%Y') AS fechafacp
                                    , s.folio AS folio
                                    , s.justificacion AS observacion
                                    , IFNULL(notifi.visto, 1) AS visto
                                    , ltdc.nombre_completo
                                    , e.estado
                                    , e.pais
                                    , z.nombre as zona
                                    , s.colabs as colabs
                                    , s.tipo_insumo
                                    , CASE 
                                        WHEN s.tipo_insumo = 1 THEN IFNULL(CONCAT('DESAYUNO (', se.diasDesayuno, ' días)'), 'DESAYUNO')
                                        WHEN s.tipo_insumo = 2 THEN IFNULL(CONCAT('COMIDA (', se.diasComida, ' días)'), 'COMIDA')
                                        WHEN s.tipo_insumo = 3 THEN IFNULL(CONCAT('CENA (', se.diasCena, ' días)'), 'CENA')
                                        WHEN s.tipo_insumo = 4 THEN CONCAT(IFNULL(CONCAT('DESAYUNO (', se.diasDesayuno, ' días)'), 'DESAYUNO'), ', ', IFNULL(CONCAT('COMIDA (', se.diasComida, ' días)'), 'COMIDA'))
                                        WHEN s.tipo_insumo = 5 THEN CONCAT(IFNULL(CONCAT('DESAYUNO (', se.diasDesayuno, ' días)'), 'DESAYUNO'), ', ', IFNULL(CONCAT('CENA (', se.diasCena, ' días)'), 'CENA'))
                                        WHEN s.tipo_insumo = 6 THEN CONCAT(IFNULL(CONCAT('COMIDA (', se.diasComida, ' días)'), 'COMIDA'), ', ', IFNULL(CONCAT('CENA (', se.diasCena, ' días)'), 'CENA'))
                                        WHEN s.tipo_insumo = 7 THEN CONCAT(IFNULL(CONCAT('DESAYUNO (', se.diasDesayuno, ' días)'), 'DESAYUNO'), ', ', IFNULL(CONCAT('COMIDA (', se.diasComida, ' días)'), 'COMIDA'), ', ', IFNULL(CONCAT('CENA (', se.diasCena, ' días)'), 'CENA'))
                                        ELSE 'N/A'
                                    END AS tipo_insumo
                                FROM solpagos s
                                LEFT JOIN lista_rtdc ltdc ON ( ltdc.idtcredito = s.idresponsable AND s.caja_chica = 2 )
                                INNER JOIN proveedores p ON(s.idProveedor = p.idProveedor)
                                LEFT JOIN ( SELECT * FROM notifi WHERE idusuario = ? ) AS notifi ON (notifi.idsolicitud = s.idsolicitud)
                                LEFT JOIN solicitud_proyecto_oficina spo ON spo.idsolicitud = s.idsolicitud
                                LEFT JOIN proyectos_departamentos pd ON spo.idProyectos = pd.idProyectos
                                LEFT JOIN solicitud_estados se on s.idsolicitud = se.idsolicitud
                                LEFT JOIN estados e on e.id_estado = se.id_estado
                                LEFT JOIN zonas z on e.idZonas = z.idZonas
                                WHERE FIND_IN_SET( s.idsolicitud, '$idsolicitudes' ) ORDER BY s.fecelab;", [$this->session->userdata("inicio_sesion")['id']]);
    }
    ...
    */
    function obtenerSolCompra_AutM( $finicio, $ffinal ) {

        $finicio = date('Y-m-d', strtotime("$finicio -2 day"));
        $ffinal = date('Y-m-d', strtotime("$ffinal -2 day"));
        ...
        return $this->db->query("SELECT
                ...
                IFNULL(f.descripcion, '') descripcion
            FROM (
                SELECT s.* FROM solpagos s
                JOIN ( SELECT departamento FROM departamentos INNER JOIN usuario_depto ON iddepartamentos = iddepartamento WHERE usuario_depto.idusuario = ? ) d 
                ON d.departamento = s.nomdepto AND s.fecha_autorizacion BETWEEN '$finicio 00:00:00' AND '$ffinal 23:59:59' AND s.caja_chica = 0 
            ) s
            CROSS JOIN proveedores p ON s.idProveedor = p.idProveedor
            CROSS JOIN etapas e ON e.idetapa = s.idetapa
            CROSS JOIN listado_usuarios capturista ON capturista.idusuario = s.idResponsable
            CROSS JOIN empresas empre ON empre.idempresa = s.idempresa
            CROSS JOIN listado_usuarios direccion_general ON direccion_general.idusuario = s.idusuario
            LEFT JOIN factura_registro f ON f.idsolicitud = s.idsolicitud
            LEFT JOIN notifi ON notifi.idsolicitud = s.idsolicitud AND notifi.idusuario = ?
            ORDER BY s.fecelab", array( $this->session->userdata("inicio_sesion")['id'], $this->session->userdata("inicio_sesion")['id'] ) );
    }

    function getTotalPagadoProv( $idproveedor ){
        
        $query = $this->db->query("SELECT 
                SUM( pagado ) pagado,
                MIN( primer_pago ) primer_pago
            FROM (
                SELECT 
                    idsolicitud 
                FROM solpagos s
                JOIN (
                    ...
                    JOIN (
                        SELECT 
                            iddfiscales
                        FROM proveedores 
                        WHERE find_in_set( idproveedor, '$idproveedor' )
                    ) dfiscales ON  p.iddfiscales = dfiscales.iddfiscales
                    UNION
                    SELECT idproveedor
                    FROM proveedores
                    WHERE find_in_set( idproveedor, '$idproveedor' )
                    AND iddfiscales IS NULL
                ) proveedores ON proveedores.idproveedor = s.idProveedor AND s.idetapa IN ( 9, 10, 11 )
            ) solpagos
            JOIN (
                SELECT 
                    ...
                    DATE_FORMAT(MIN( autpagos.year_registro ),'%Y-%m-%d') AS primer_pago
                FROM
                (
                    SELECT 
                        autpagos.idsolicitud AS idsolicitud,
                        ...
                        autpagos.fecreg AS year_registro
                    FROM autpagos
                    WHERE (autpagos.referencia <> 'ABONO')
                    AND estatus IN ( 14, 15, 16 )
                    AND idsolicitud IS NOT NULL
                    UNION 
                    SELECT 
                        autpagos.idsolicitud AS idsolicitud,
                        autpagos.cantidad AS cantidad,
                    ...
                    FROM
                        autpagos
                    WHERE ISNULL(autpagos.referencia)
                    AND estatus IN ( 14, 15, 16 )
                    AND idsolicitud IS NOT NULL
                ) autpagos
                GROUP BY autpagos.idsolicitud
            ) TEMP_total_pagado_proveedor ON TEMP_total_pagado_proveedor.idsolicitud = solpagos.idsolicitud");
        if( $query && count( $query->result_array() ) ){

            $query = $query->result_array()[0];

            if( $query["primer_pago"] ){
                setlocale(LC_TIME, 'es_ES', 'Spanish_Spain', 'Spanish');
                $fecha_primier_pago = explode( "-", $query["primer_pago"] );
            }else{
                $fecha_primier_pago = explode( "-", '2000-01-01' );
            }
            ...
            return [
                "estatus" => 1,
                "mjs" => "Total pagado $".number_format( $query["pagado"], 2, ".", "," )." desde el ".(integer)$fecha_primier_pago[2]." de ".$meses[ $fecha_primier_pago[ 1 ] - 1 ]." del ".$fecha_primier_pago[ 0 ]
            ];
        }else 
        return [
            "estatus" => 0,
            "mjs" => "Este proveedor no cuenta con pagos realizados."
        ];
    }
