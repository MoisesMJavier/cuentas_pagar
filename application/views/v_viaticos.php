		...
		<div class="col-lg-12">
			<div class="box">
				<div class="box-header">
					<h3>DIRECCIÓN GENERAL VIÁTICOS</h3>
				</div>
				<div class="box-body">
					...
					<hr>
					<table class="table table-responsive table-bordered table-striped table-hover" id="solViaticos">
						<thead>
						<tr>
							<th></th>
							<th><i style="cursor: pointer;" class="fa fa-check-square seleccionar_todas text-red" title="Seleccionar todas los viáticos."></i></th> <!-- FECHA: 17-DICIEMBRE-2024 | @author Angel Victoriano <programador.analista30@ciudadmaderas.com> -->
		...
		<div class="modal-content">
			<form id="formularioCJ">
				<div class="modal-header">
					...
					<h4 class="modal-title">DECLINAR PAGO VIÁTICOS</h4>
				</div>
...
<script type="text/javascript">
	...
	$(".seleccionar_todas").click( function(){
		...
		$( tabla_viaticos.$('input[type="checkbox"]')).each( function( i, v ){

			if( !$(this).prop("checked") ){
				$(this).prop("checked", true);
				tota2 += parseFloat( tabla_viaticos.row( $(this).closest('tr') ).data().Cantidad );
			}else{
	...
	$("#solViaticos").ready( function () {

		$('#solViaticos thead tr:eq(0) th').each( function (i) {
			if( i != 0 && i!=1 && i!=5 && i != 7){
				var title = $(this).text();
		...
		tabla_viaticos = $('#solViaticos').DataTable({
			dom: 'rtip',
			"language" : lenguaje,
			"processing": false,
			"pageLength": 20,
		...
		$('#solViaticos').on( "click", ".btn-recharzarPagoCH", function( e ){
		...
		$('#solViaticos').on( 'click', 'input', function () {
			tr = $(this).closest('tr');
			var row = tabla_viaticos.row( tr );
	...
	function autorizarSeleccionadasCajaChica(){
		...
		$( tabla_viaticos.$('input[type="checkbox"]:checked')).each(function(index,va){
		...
		if(window.confirm('Se pagará el total autorizado.\nEl total es de $ '+ formatMoney(tota2)+' ¿Estás de acuerdo?')){
			$.post( url + "DireccionGeneral/PagarTotalReembolsos", {jsonApagar : JSON.stringify(apagar)} ).done( function( data ){
				...
				if( data.resultado ){
					...
					tabla_viaticos.clear();
					tabla_viaticos.rows.add( data.data );
					tabla_viaticos.draw();
				}else{
	...
	function format ( d ) {
		...
		var solicitudes = '<div style="width: 100%; padding-top: 10px; padding-bottom: 10px;">';
		$.each(d.solicitudes, function(i, v) {
			solicitudes += `
								<div class="row col-xs-11 col-md-11">
									...
									<div class="col-12 col-md-4">
										<p><b>ETAPA:</b> ${v.etapa}</p>
										<p><b>FECHA FACTURA:</b> ${v.fechafacp}</p>

										<p><b>ZONA:</b> ${v.zona ? v.zona : 'NA'}</p>
										<p><b>TIPO INSUMO:</b> ${v.tipo_insumo ? v.tipo_insumo : 'NA'}</p>
									</div>
									<div class="col-12 col-md-4">
										<p><b>CONDOMINIO:</b> ${v.condominio}</p>
										<p><b>CANTIDAD:</b> ${formatMoney(v.cantidad)}</p>

										<p><b>ESTADO:</b> ${v.estado ? v.estado : 'NA'}</p>
									</div>
									<div class="col-12 col-md-12">
										<p><b>JUSTIFICACIÓN:</b> ${v.observacion}</p>
									</div>
								</div>
								<div class="row mt-2">
			...
			solicitudes += ` ${(i < d.solicitudes.length - 1 ? '<hr style="border-top: 1px solid #E4E4E4; margin-top: 10px; margin-bottom: 10px;">' : '')}`;
		});
		solicitudes +=    '</div>';
		return solicitudes;
	}
