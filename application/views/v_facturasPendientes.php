        ...
        <div class="col-lg-12">
            <div class="box">
                <div class="box-header">
                    <h3>DIRECCIÓN GENERAL OTROS PAGOS (FINIQUITOS, PRÉSTAMOS Y NÓMINA)</h3>                 
                </div>
                <div class="box-body">
   ...
   $("#solPagosPen").ready( function () {
        ...
        tabla_getion_pagosP = $('#solPagosPen').DataTable({
           ...
           //             }
                    ...
                    "data" : function( d ){
                        ...
                        var opciones = `
                            <div class="btn-group-vertical">
                                <button id="verSol" name="verSol" class="btn btn-instagram consultar_modal notification btn-sm" value="${d.ID}" data-value="BAS" data-toggle="tooltip" data-placement="bottom" title="Ver Solicitud">
                                    <i class="fas fa-eye" ></i>${d.visto == 0 ? '</i><span class="badge">!</span>' : ''}
                                </button>
                                ${!idusuario ? `
                                    <button id="rechazado" name="rechazado" value="${d.ID}" class="btn btn-danger btn-recharzarPago btn-sm" title="Declinar Pago">
            ...
            "ajax": url + "DireccionGeneral/tablaDGotrosgastos"
            
        });

        if (!idusuario) { /** FECHA: 17-DICIEMBRE-2024 | @author Angel Victoriano <programador.analista30@ciudadmaderas.com> */
            tabla_getion_pagosP.column(1).visible(true);
        }else{
            tabla_getion_pagosP.column(1).visible(false);
