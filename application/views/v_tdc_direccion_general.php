        ...
        <div class="col-lg-12">
            <div class="box">
                <div class="box-header">
                     <h3>GASTOS DE TARJETAS DE CREDITO</h3>
                </div>
                <div class="box-body">
                    <?php if ($this->session->userdata('inicio_sesion')["id"] !== '2636'): ?> <!-- INICIO FECHA: 17-DICIEMBRE-2024 | @author Angel Victoriano <programador.analista30@ciudadmaderas.com> -->
                        <div class="row">
                            ...
                            <div class="col-lg-6 col-md-6 text-right">
                                <div class="btn-group">  
                                    <button class="btn btn-info" onclick="autorizarSeleccionadasCajaChica()">AUTORIZAR SELECCIONADAS</button>
                                </div>
                            </div>
                        </div>
                    <?php endif; ?> <!-- FIN FECHA: 17-DICIEMBRE-2024 | @author Angel Victoriano <programador.analista30@ciudadmaderas.com> -->
                    <hr>
                    <table class="table table-responsive table-bordered table-striped table-hover" id="solCajaChica">
                        <thead>
                            <tr>
                                <th></th>
                                <th><i style="cursor: pointer;" class="fa fa-check-square seleccionar_todas text-red" tittle="Seleccionar todas las tdc."></i></th> <!-- FECHA: 17-DICIEMBRE-2024 | @author Angel Victoriano <programador.analista30@ciudadmaderas.com> -->
                                <th style="font-size: .9em">BANCO TARJETA</th>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-header">
                ...
                <h4 class="modal-title">DECLINAR PAGO C.CHICA</h4>
            </div>
...
<script type="text/javascript">
    ...
    $(".seleccionar_todas").click( function(){
        ...
        $( tabla_caja_chica.$('input[type="checkbox"]')).each( function( i, v ){
        
            if( !$(this).prop("checked") ){
                $(this).prop("checked", true);
                tota2 += parseFloat( tabla_caja_chica.row( $(this).closest('tr') ).data().cantidad );    
            }else{
    ...
    $("#solCajaChica").ready( function () {
 
         $('#solCajaChica thead tr:eq(0) th').each( function (i) {
            if( i != 0 && i!=1 && i < $('#solCajaChica thead tr:eq(0) th').length ){
                var title = $(this).text();
        ...
        tabla_caja_chica = $('#solCajaChica').DataTable({
            dom: 'rtip',
            "language" : lenguaje,
            "processing": false,
            "pageLength": 20,
        ...
        $('#solCajaChica').on( "click", ".btn-recharzarPagoCH", function( e ){
        ...
        $('#solCajaChica').on( 'click', 'input', function () {
            tr = $(this).closest('tr');
            var row = tabla_caja_chica.row( tr );
    ...
    function autorizarSeleccionadasCajaChica(){
        ...
        $( tabla_caja_chica.$('input[type="checkbox"]:checked')).each(function(index,va){
          ...
          if(window.confirm('Se pagará el total autorizado.\nEl total es de $ '+ formatMoney(tota2)+' ¿Estás de acuerdo?')){   
            $.post( url + "DireccionGeneral/PagarTotalCajaChica", {jsonApagar : JSON.stringify(apagar)} ).done( function( data ){
                    ...
                    if( data.resultado ){
                        ...
                        tabla_caja_chica.rows.add( data.data );
    ...
    function format ( d ) {
        ...
        solicitudes += '<tr>';
        solicitudes += '<th><b>'+'# SOLICITUD'+'</b></th>'; // @author Efrain Martinez Muñoz creación de columna solicitud 08/08/2024 
        ...
        $.each( d.solicitudes, function( i, v){ 
            ...
            solicitudes += '<td>'+v.idsolicitud+'</td>';
            ...
            solicitudes += '<td colspan="2"><b>'+'RESPONSABLE '+'</b>'+v.nombre_completo+'</td>';
            solicitudes += '</tr>';
        });          
